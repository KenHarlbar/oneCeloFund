...
import onedollarfundAbi from "../contracts/onedollarfund.abi.json"

const ContractAddress = "0x5334EA34BCADA7f9559aF476c38727515BCD7F1F";
let kit
let contract
let campaigns = []
...
const connectCeloWallet = async function () {
    if (window.celo) {
        ...
        try {
            ...
            contract = new kit.web3.eth.Contract(onedollarfundAbi, ContractAddress)
        } catch (error) {
...
function renderCampaigns() {
    document.getElementById("onedollarfund").innerHTML = ""
    campaigns.forEach((_campaign) => {
        const newDiv = document.createElement("div")
        newDiv.className = "col-md-4"
        newDiv.innerHTML = campaignTemplate(_campaign)
        document.getElementById("onedollarfund").appendChild(newDiv)
    })
}
...
document.querySelector("#onedollarfund").addEventListener("click", async (e) => {
